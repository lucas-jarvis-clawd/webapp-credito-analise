...
class DataSeeder {
    ...
    generateDuplicatas(clientesIds, count) {
        const duplicatas = [];
        const hoje = new Date();
    ...
    generatePagamentos(duplicatasPagas) {
        const pagamentos = [];
          ...
          updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            ...
            for (const tableSQL of tables) {
                try {
                    ...
                    logger_1.logger.info('Tabela criada com sucesso');
                }
                catch (error) {
                    if (error.message && error.message.includes('ORA-00955')) {
                        logger_1.logger.info('Tabela já existe, continuando...');
                    }
                    else {
                        throw error;
                    }
    ...
    async seedDatabase(clientesCount = 50, duplicatasCount = 300) {
        logger_1.logger.info('🌱 Iniciando seed do banco de dados...');
        await this.createTables();
        const connection = await (0, connection_1.getConnection)();
        try {
            // 1. Inserir clientes
            logger_1.logger.info('📊 Inserindo clientes...');
        ...
        `, {
            ...
            logger_1.logger.info(`✅ ${clienteIds.length} clientes inseridos`);
            // 2. Inserir duplicatas
            logger_1.logger.info('💳 Inserindo duplicatas...');
            const duplicatas = this.generateDuplicatas(clienteIds, duplicatasCount);
            const duplicatasPagas = [];
        ...
        `, {
            ...
            logger_1.logger.info(`✅ ${duplicatas.length} duplicatas inseridas`);
            // 3. Inserir pagamentos
            logger_1.logger.info('💰 Inserindo pagamentos...');
        ...
        `, pagamento);
            ...
            logger_1.logger.info(`✅ ${pagamentos.length} pagamentos inseridos`);
            // 4. Inserir configurações padrão
            logger_1.logger.info('⚙️ Inserindo configurações...');
            const configs = [
          ...
          `, config);
            ...
            logger_1.logger.info('✅ Configurações inseridas');
            logger_1.logger.info('🎉 Seed do banco de dados concluído com sucesso!');
        }
        finally {
            await connection.close();
        }
...
if (require.main === module) {
    ...
    seeder.seedDatabase()
        .then(() => {
        logger_1.logger.info('✨ Processo de seed finalizado');
        process.exit(0);
    })
        .catch((error) => {
        logger_1.logger.error('❌ Erro no seed:', error);
        process.exit(1);
    });
}
