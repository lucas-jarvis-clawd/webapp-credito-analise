...
import { ScoreMetrics, ScoreWeights, ScoreResult } from '../models/types';
import { logger } from '../utils/logger';

export class ScoreService {

  private getDefaultWeights(): ScoreWeights {
    return {
  ...
  async calculateClientMetrics(clienteId: number): Promise<ScoreMetrics> {
    ...
    try {
      ...
      const stats = result.rows && result.rows.length > 0 ? result.rows[0] as any : null;
      ...
      logger.error('Erro ao calcular métricas do cliente:', error);
      throw error;
    } finally {
  ...
  private calculatePontuacaoInterna(stats: any): number {
  ...
  private normalizeValorMedio(valorMedio: number): number {
  ...
  private normalizePrazoMedio(prazoMedioDias: number): number {
  ...
  async calculateScore(clienteId: number, weights?: ScoreWeights): Promise<ScoreResult> {
    try {
      ...
      const scoreFinal = Math.round(
        (metricas.pontuacao_interna * ponderacoes.pontuacao) +
  ...
  private getClassificacao(score: number): 'EXCELENTE' | 'BOM' | 'REGULAR' | 'RUIM' | 'PÉSSIMO' {
  ...
  private async saveScoreResult(result: ScoreResult): Promise<void> {
    ...
    try {
      ...
      logger.error('Erro ao salvar resultado do score (não crítico):', error);
    } finally {
      await connection.close();
    }
  }
  ...
  async getScoreHistory(clienteId: number, limit: number = 10): Promise<ScoreResult[]> {
    ...
    try {
      ...
      logger.error('Erro ao buscar histórico de scores:', error);
      throw error;
    } finally {
      await connection.close();
    }
